...
export async function forPerson(personId: string): Promise<PersonBehaviour> {
  const rows = (await db('access_events as e')
  ...
  const scored = rows.filter((r) => r.anomaly_score !== null).map(toScored);
  const profile = await behavior.profile(personId);
...
export async function seed(
  ...
  const isEntry = (code: string) => /MAIN|ENTRY|LOBBY|ULAZ/i.test(code);
