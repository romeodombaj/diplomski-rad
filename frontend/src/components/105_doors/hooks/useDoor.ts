...
export function useDoor() {
  const [data, setData]         = useState<Door[]>([]);
  ...
  useEffect(() => {
    ...
    setError(null);
    setHasMore(false);
    DoorService.getAll(search, cursor, LIMIT, withCount, filters, sortField, sortDir, pageIndex + 1)
      .then((res) => {
        setData(res.data);
  ...
  const remove = (id: number) => DoorService.remove(id).then(() => {
    cursors.current = [null];
    setTotal(undefined);
    setPageIndex(0);
